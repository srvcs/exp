...
use srvcs_exp::{api::Deps, health, router, telemetry};
...
/// genuinely exercises exp's validation branch (number -> proceed, non-number
...
/// Pull the `result` field as an f64 for approximate comparison.
fn result_f64(body: &Value) -> f64 {
    body.get("result")
        .and_then(Value::as_f64)
        .expect("result is a JSON number")
}
...
async fn exp_of_one_is_e() {
    ...
    assert_eq!(status, StatusCode::OK);
    // e ~= 2.718281828459045
    assert!((result_f64(&body) - std::f64::consts::E).abs() < 1e-9);
}

#[tokio::test]
async fn exp_accepts_floats() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(2.5)).await;
    assert_eq!(status, StatusCode::OK);
...
#[tokio::test]
async fn exp_of_negative() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(-1)).await;
    assert_eq!(status, StatusCode::OK);
    assert!((result_f64(&body) - (1.0 / std::f64::consts::E)).abs() < 1e-9);
}
...
async fn rejects_when_isnumber_says_false() {
    ...
    let (status, _) = eval(&isnumber, json!(1)).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
}

#[tokio::test]
async fn forwards_dependency_422() {
    // Leaf dependency rejects input; exp forwards the 422 verbatim.
